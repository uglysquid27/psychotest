...
export default function KraepelinSimple({ testConfig }) {
    ...
    const TEST_ID = testConfig?.testId || null;
    ...
    const [isTestCompleted, setIsTestCompleted] = useState(testConfig?.isCompleted || false);
    const [hasAttempted, setHasAttempted] = useState(testConfig?.hasAttempted || false);
    ...
    useEffect(() => {
        ...
        if (testConfig?.isCompleted) {
            ...
            if (testConfig?.testData) {
                loadExistingTestData(testConfig.testData);
            }
        }
    ...
    const loadExistingTestData = (data) => {
        setNumberMatrix(data.number_matrix || []);
        ...
        setRowPerformance(data.row_performance || []);
        setColumnPerformance(data.column_performance || []);
        setPerformanceData(data.performance_data || []);
        
        if (data.answer_matrix) {
            setAnswerMatrix(data.answer_matrix);
        } else {
            const matrix = calculateAnswerMatrix(data.user_answers, data.number_matrix);
            setAnswerMatrix(matrix);
        }
    };

    const initializeTest = async () => {
        // Don't initialize if test is already completed
        if (isTestCompleted) {
            setIsLoading(false);
            return;
        }
    ...
    useEffect(() => {
        if (keypadRef.current && !isFinished && !isTestCompleted) {
            keypadRef.current.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }
    }, [currentRow, currentCol, isFinished, isTestCompleted]);
    ...
    }, [isTimerRunning, timeLeft, isLoading, isTestCompleted]);

    const startTest = async () => {
        if (isTestCompleted || hasAttempted) {
            return;
        }
    ...
    const handleNumberClick = (digit) => {
        if (isLoading || isTestCompleted) return;
        if (!isTimerRunning && !testStarted) startTest();
    ...
    const calculateAnswerMatrix = (answers = userAnswers, matrix = numberMatrix) => {
    ...
    const submitTest = async () => {
        if (isFinished || isSubmitting || isTestCompleted) return;
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (testId) {
                    try {
                        await axios.post(route('employee.test-assignments.complete', 'kraepelin'), {
                            score: response.data.correctAnswers || score.correct,
                        ...
                        console.error('Failed to mark assignment as completed:', e);
                    }
                }
            }
        } catch (error) {
                                ...
                                return (
                                    <tr 
                                        ...
                                        className={`transition-colors duration-200 ${
                                            rowIndex === currentRow && isTimerRunning && !isTestCompleted
                                    ...
                                    >
                                        {row.map((num, colIndex) => {
                                            ...
                                            const isCurrentCell = isActiveColumn && rowIndex === currentRow && isTimerRunning && !isTestCompleted;
                                            const isPairCell = isActiveColumn && rowIndex === currentRow + 1 && isTimerRunning && !isTestCompleted;
                                            ...
                                            return (
                                                <td
                                                    ...
                                                    className={`text-center p-2 border border-slate-300
                                                        ...
                                                        ${showAnswer && !isCurrentCell && !isPairCell && !isTestCompleted
                                                        ...
                                                        ${!isActiveColumn && !isBottomRow && !isCurrentCell && !isPairCell && !isTestCompleted
                                                            ? 'opacity-40' 
                                                            : isTestCompleted
                                                            ? 'bg-slate-50'
                                                            : ''
                                                ...
                                                >
                                                    <div className="flex items-center justify-center gap-1 md:gap-2 w-full h-full">
                                                        ...
                                                        {isTestCompleted && userAnswers[rowIndex]?.[colIndex] !== null && (
                                                            <span className={`font-bold text-base sm:text-lg md:text-xl ${
                                                                userAnswers[rowIndex][colIndex] === 
                                                                ((num + (numberMatrix[rowIndex + 1]?.[colIndex] || 0)) % 10)
                                                                    ? 'text-emerald-600'
                                                                    : 'text-rose-600'
                                                            }`}>
                                                                {userAnswers[rowIndex][colIndex]}
                ...
                <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
                    <table className="mx-auto border-collapse">
                        <thead className="sticky top-0 bg-slate-100 z-10">
                            <tr>
                                <th className="p-2 text-slate-500 text-sm font-medium border border-slate-300 bg-slate-100">Baris/Kolom</th>
                                    ...
                                    {row.slice(0, 30).map((cell, cellIdx) => (
                                        <div
                                            ...
                                            className={`
                                                ...
                                                hover:ring-1 hover:ring-slate-400 transition-all
                                            `}
                                            title={`Row ${idx + 1}, Col ${cellIdx + 1}: ${cell.status === 'correct' ? 'Correct' : cell.status === 'wrong' ? 'Wrong' : 'Unanswered'}`}
                                        />
                                    ))}
                                    {COLS > 30 && (
    ...
    const renderTestCompletedMessage = () => {
        return (
            <div className="max-w-6xl mx-auto px-4 md:px-6">
                <div className="fade-in-up soft-beige-shadow bg-white rounded-3xl shadow-2xl overflow-hidden border-2 border-amber-100">
                    <div className="bg-gradient-to-r from-amber-600 via-orange-600 to-amber-600 p-6 md:p-10 text-center">
                        <div className="inline-flex items-center justify-center w-20 h-20 md:w-24 md:h-24 bg-white/20 rounded-full mb-4 md:mb-6">
                            <svg className="w-10 h-10 md:w-14 md:h-14 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <h1 className="text-3xl md:text-4xl font-bold text-white mb-2" style={{fontFamily: "'Raleway', sans-serif"}}>
                            Tes Sudah Diselesaikan
                        </h1>
                        <p className="text-lg text-amber-100">
                            Anda telah menyelesaikan tes Kraepelin ini sebelumnya
                        </p>
                    </div>
                    ...
                    <div className="p-6 md:p-10 space-y-6">
                        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center">
                            <p className="text-slate-700 mb-4">
                                Setiap tes hanya dapat dikerjakan satu kali. Jika Anda membutuhkan tes ulang, silakan hubungi administrator.
                            </p>
                            <button
                                onClick={handleGoToMyAssignments}
                                className="px-8 py-4 bg-gradient-to-r from-amber-600 to-orange-600 text-white font-bold rounded-xl shadow-lg hover:shadow-2xl hover:from-amber-700 hover:to-orange-700 transition-all duration-300 transform hover:-translate-y-1 inline-flex items-center gap-2"
                            >
                        ...
                        {answerMatrix.length > 0 && (
                            <div className="mt-8 space-y-6">
                                <h2 className="text-xl font-semibold text-slate-800 mb-4 text-center" style={{fontFamily: "'Raleway', sans-serif"}}>
                                    Hasil Tes Anda
                                </h2>
    ...
    if (isTestCompleted && isFinished) {
        return (
            <AuthenticatedLayout
                header={
                    <h2 className="text-xl font-semibold leading-tight text-gray-800">
                        Tes Kraepelin - {getDifficultyLabel()} (Selesai)
                    </h2>
                }
            >
            ...
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800">
                    ...
                    <span className="text-sm font-normal text-gray-600 ml-3">
                        ...
                        {hasAttempted && !isTestCompleted && (
                            <span className="ml-3 text-amber-600 font-semibold">
                                (Percobaan {testConfig?.attempts || 1})
                            </span>
                        )}
                    </span>
                </h2>
                ...
                {showGuide ? (
                    <div className="max-w-4xl mx-auto px-4 md:px-6">
                        <div className="fade-in-up soft-beige-shadow bg-white rounded-3xl shadow-2xl overflow-hidden border-2 border-amber-100">
                            ...
                            <div className="p-6 md:p-10 space-y-6 md:space-y-8">
                                    ...
                                    <div className="flex items-start gap-3 md:gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs md:text-sm text-yellow-800 leading-relaxed" style={{fontFamily: "'Inter', sans-serif"}}>
                                                ...
                                                <br />
                                                <span className="font-semibold text-amber-700">Tes ini hanya dapat dikerjakan SATU KALI.</span> Pastikan Anda siap sebelum memulai.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <button
                                    onClick={startTest}
                                    disabled={isTestCompleted || hasAttempted}
                                    className={`w-full py-5 md:py-6 bg-gradient-to-r from-amber-600 via-orange-600 to-amber-600 text-white text-lg md:text-xl font-bold rounded-2xl shadow-lg hover:shadow-2xl hover:from-amber-700 hover:via-orange-700 hover:to-amber-700 transition-all duration-300 transform hover:-translate-y-1 flex items-center justify-center gap-3 md:gap-4
                                        ${(isTestCompleted || hasAttempted) ? 'opacity-50 cursor-not-allowed' : ''}`}
                    ...
                    <div className="max-w-6xl mx-auto px-4 md:px-6">
                        <div className="fade-in-up soft-beige-shadow bg-white rounded-3xl shadow-2xl p-4 md:p-8 border-2 border-amber-100">
                            ...
                            <div 
                                ref={containerRef}
                                className="soft-beige-shadow bg-gradient-to-br from-slate-50 to-gray-50 rounded-2xl p-4 md:p-6 mb-6 md:mb-8 border border-slate-200" 
                    ...
                    <div className="max-w-6xl mx-auto px-4 md:px-6">
                        <div className="fade-in-up soft-beige-shadow bg-white rounded-3xl shadow-2xl overflow-hidden border-2 border-amber-100">
                            ...
                            <div className="p-6 md:p-10 space-y-6 md:space-y-8">
                                ...
                                {/* Heatmaps */}
                                <div className="fade-in-up grid grid-cols-1 lg:grid-cols-2 gap-6" style={{animationDelay: '0.15s'}}>
                                    {renderRowHeatmap()}
                                    {renderColumnHeatmap()}
                                </div>
                                ...
                                <div className="fade-in-up flex justify-center gap-3 md:gap-4 mb-6 md:mb-8" style={{animationDelay: '1.1s'}}>
                                    <button
                                        onClick={handleGoToMyAssignments}
...
function getEncouragingMessage() {
    const progress = Math.random(); // You can make this dynamic based on actual progress
    const accuracy = Math.random() * 100; // You can make this dynamic based on actual accuracy
