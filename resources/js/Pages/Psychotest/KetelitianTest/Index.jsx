// resources/js/Pages/Psychotest/KetelitianTest/Index.jsx (ADMIN QUESTION LIST)
import { useState, useEffect } from 'react';
import { Head, Link, router, usePage } from '@inertiajs/react';
import AuthenticatedLayout from '@/Layouts/AuthenticatedLayout';
import Pagination from '@/Components/Pagination';

export default function KetelitianQuestionList({ questions = null, filters = {} }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('admin.ketelitian.questions.index'), {
            search: search,
            per_page: perPage
        }, {
            preserveState: true,
    ...
    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this question?')) {
            router.delete(route('admin.ketelitian.questions.destroy', id));
        }
    };
    ...
    const getAnswerBadge = (answer) => {
        const isSame = answer === 'S';
        return (
            <span className={`px-3 py-1 rounded-full text-sm ${isSame ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                {isSame ? 'SAMA' : 'TIDAK SAMA'}
            </span>
        );
    };
    ...
    return (
        ...
        >
            <Head title="Kelola Soal Ketelitian" />
                                        ...
                                        {questionsData.map((question) => (
                                            <tr key={question.id}>
                                                <td className="px-4 py-3">
                                                    <div className="max-w-xs truncate">{question.left_text}</div>
                                                </td>
                                                <td className="px-4 py-3">
                                                    <div className="max-w-xs truncate">{question.right_text}</div>
                                                </td>
                                                <td className="px-4 py-3">
                                                ...
                                                <td className="px-4 py-3 space-x-2">
                                                    <Link
                                                        href={route('admin.ketelitian.questions.edit', question.id)}
