...
useEffect(() => {
  if (questionRefs.current[currentIndex] && questionsContainerRef.current) {
    ...
    if (window.innerWidth >= 1024) { // desktop/lg
      ...
      // Hitung scroll agar soal terlihat tapi tidak terlalu ke atas
          ...
          {leftColumn.map((q, idx) => {
            ...
            return (
              <div
                ...
                className={`p-3 mb-2 rounded-md border transition-colors cursor-pointer
                  ${globalIndex === currentIndex ? 'bg-yellow-100 border-yellow-400' :
                    userAnswers[globalIndex] ?
                      (userAnswers[globalIndex] === q.answer ? 'bg-green-100 border-green-300' : 'bg-red-100 border-red-300') :
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  {userAnswers[globalIndex] && (
                    <span className={`font-bold ${userAnswers[globalIndex] === q.answer ? 'text-green-600' : 'text-red-600'}`}>
                      {userAnswers[globalIndex]}
                    </span>
                  )}
                </div>
