...
const validateAndConvertImage = async (file) => {
    // Validasi tipe file
    const validTypes = [
        "image/jpeg",
        "image/jpg",
...
export default function Index() {
    ...
    const handleFileSelect = async (e) => {
        ...
        try {
            ...
            // Konversi file ke PNG
            const conversionResult = await validateAndConvertImage(file);
            const pngFile = conversionResult.file;

            setUploadStatus("Uploading to ImageKit...");

            // Upload ke ImageKit menggunakan API mereka
            ...
            formData.append("fileName", `equipment_${Date.now()}.png`);
            ...
            if (uploadResponse.ok && uploadResult.url) {
                setForm({ ...form, photo: uploadResult.url });
    ...
    const getAvailableStock = () => {
        ...
        if (selectedEquipment.size) {
            // Check if size is not null or empty
            if (!selectedEquipment.size.trim()) return 0;

            const selectedSizeData = selectedEquipment.size
                .split(",")
                .find((s) => {
                    // Add null check for each size item
            ...
            return parseInt(selectedEquipment.amount) || 0;
        }
    };
    ...
    const handleEmployeeSelect = (employee) => {
        ...
        if (isCurrentlySelected) {
            ...
            if (currentTotalSelected >= availableStock) {
                showNotification(
                    `Cannot select more than available stock (${availableStock})!`,
                    "error"
                );
                return;
            }
    ...
    const handleSelectAll = () => {
        if (isSelectAll) {
            ...
            const availableEmployees = employees.data.filter(
                (emp) => !allSelectedEmployees.has(emp.id)
            );
            ...
            const employeesToSelect = availableEmployees.slice(
                0,
                remainingStock
            );
            ...
            if (availableEmployees.length > remainingStock) {
                showNotification(
                    `Only ${remainingStock} additional employees selected due to stock limitation`,
                    "warning"
                );
            }
    ...
    const handleAssignMultiple = async () => {
        ...
        if (availableStock < allSelectedEmployees.size) {
            showNotification(
                `Cannot assign: Only ${availableStock} items available, but ${allSelectedEmployees.size} employees selected!`,
                "error"
            );
            return;
        }
        ...
        try {
            ...
            if (failedAssignments.length === 0) {
                showNotification(
                    `Successfully assigned equipment to ${successfulAssignments.length} employees!`,
                    "success"
                );
                ...
                showNotification(
                    `Assigned to ${successfulAssignments.length} employees, but ${failedAssignments.length} failed.`,
                    "warning"
                );
            ...
            console.error(
                "Error assigning equipment to multiple employees:",
                error
            );
            showNotification(
                "Error assigning equipment: " + error.message,
                "error"
            );
        } finally {
    ...
    const loadEmployees = async (
        ...
        try {
            const payload = {
                ...
                size: size,
                search:
                    customSearch !== undefined ? customSearch : filters.search,
                section:
                    customSection !== undefined
                        ? customSection
                        : filters.section,
                subsection:
                    customSubsection !== undefined
                        ? customSubsection
                        : filters.subsection,
                page: page,
            };
    ...
    const handleAssignToEmployee = async (employee) => {
        ...
        if (availableStock <= 0) {
            showNotification(
                "Cannot assign equipment: Stock is out of stock!",
                "error"
            );
            return;
        }
        ...
        try {
            const response = await fetch(
                route("equipments.assign.store.modal"),
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (result.success) {
                ...
                window.location.reload();
                showNotification(
                    `Equipment assigned to ${employee.name}!`,
                    "success"
                );
                setAssignPhoto("");
            } else {
                ...
                <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
                            ...
                            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                                <div>
                                    ...
                                    <p className="text-gray-600 dark:text-gray-300">
                                        Manage and assign work equipment to
                                        employees
                                    </p>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {equipments.map((eq, index) => (
                            <div
                                key={eq.id}
                                className="bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-700 group"
                            >
                                {/* Equipment Image */}
                                                        ...
                                                        eq.size
                                                            ...
                                                            .map(
                                                                (
                                                                    ...
                                                                    // Add null/empty check for each size item
                                                                    if (
                                                                        !sizeItem ||
                                                                        !sizeItem.includes(
                                                                            ":"
                                ...
                                {!form.photo ? (
                                    <div className="space-y-4">
                                        ...
                                        >
                                            ...
                                            Choose Photo (Auto-convert to PNG)
                                        </label>
                                    ...
                                    <div className="space-y-4">
                                        <p className="text-sm font-medium text-green-600 dark:text-green-400">
                                            ✓ Photo uploaded successfully as
                                            PNG!
                                        </p>
                                        <img
                                            src={form.photo}
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    >
                                        {isSubmitting
                                            ? "Assigning..."
                                            : `Assign to ${allSelectedEmployees.size} Employees`}
                                    </button>
                                </div>
                            </div>
                        </div>
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                                    ...
                                    employees.data.length > 0 ? (
                                        employees.data.map((employee) => {
                                            const isSelected =
                                                allSelectedEmployees.has(
                                                    employee.id
                                                );
                                            const hasExistingAssignment =
                                                employee.handovers &&
                                                employee.handovers.length > 0;
                                            const existingAssignment =
                                                hasExistingAssignment
                                                    ? employee.handovers[0]
                                                    : null;
                            ...
                            employees.meta.last_page > 1 && (
                                <div className="flex justify-between items-center mt-4 px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
                                    <div className="text-sm text-gray-700 dark:text-gray-300">
                                        Showing {employees.data.length} of{" "}
                                        {employees.meta.total} employees
                                    </div>
                                    <div className="flex gap-1">
